...
function! s:steelydylan() "{{{1
    hide enew
    setlocal buftype=nofile nowrap nolist nonumber bufhidden=wipe
    setlocal modifiable nocursorline nocursorcolumn

    let steelydylan = []
    call add(steelydylan, 'Copyright (c) 2014                                 steelydylan''s vimrc.')
    call add(steelydylan, '.______    _  _    .______    _  _    .______        ___    ______  ')
    call add(steelydylan, '|   _  \  | || |   |   _  \  | || |   |   _  \      / _ \  |____  | ')
    call add(steelydylan, '|  |_)  | | || |_  |  |_)  | | || |_  |  |_)  |    | | | |     / /  ')
    call add(steelydylan, '|   _  <  |__   _| |   _  <  |__   _| |      /     | | | |    / /   ')
    call add(steelydylan, '|  |_)  |    | |   |  |_)  |    | |   |  |\  \----.| |_| |   / /    ')
    call add(steelydylan, '|______/     |_|   |______/     |_|   | _| `._____| \___/   /_/     ')
    call add(steelydylan, '           #VIM + #ZSH + #TMUX = Best Developer Environmen          ')

    silent put =repeat([''], winheight(0)/2 - len(steelydylan)/2)
    let space = repeat(' ', winwidth(0)/2 - strlen(steelydylan[0])/2)
    for line in steelydylan
        put =space . line
    endfor
    silent put =repeat([''], winheight(0)/2 - len(steelydylan)/2 + 1)
    silent file steelydylan
    1
...
"     autocmd!
"     autocmd VimEnter * if !argc() | call <SID>steelydylan() | endif
" augroup END
